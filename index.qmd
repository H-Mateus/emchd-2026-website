---
title: "EMCHD 2026"
title-block-banner: images/banner.png
#include-after-body: countdown.html
---

::: {.column-margin}

<div style="border: 2px solid #00693E; border-radius: 8px; padding: 0.8em; background-color: #f9f9f9;">

<div style="font-size:1.3em; font-weight:bold;">
  <span style="color:#D30707;">Countdown to EMCHD 2026:</span><br>
  <span id="countdown" style="color:#00693E;"></span>
</div>

<script>
// Set the target date (YYYY-MM-DDTHH:MM:SS)
var countDownDate = new Date("2026-09-01T09:00:00").getTime();

var countdownFunction = setInterval(function() {
  var now = new Date().getTime();
  var distance = countDownDate - now;

  if (distance < 0) {
    document.getElementById("countdown").innerHTML = "The conference has started!";
    clearInterval(countdownFunction);
    return;
  }

  var days = Math.floor(distance / (1000 * 60 * 60 * 24));
  var hours = Math.floor((distance % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60));
  var minutes = Math.floor((distance % (1000 * 60 * 60)) / (1000 * 60));
  var seconds = Math.floor((distance % (1000 * 60)) / 1000);

  document.getElementById("countdown").innerHTML =
    days + "d " + hours + "h " + minutes + "m " + seconds + "s";
}, 1000);
</script>

<div style="border-top: 1px solid #00693E; margin: 0.6em 0;"></div>

<div style="font-size:1.3em; font-weight:bold;">
  <span style="color:#D30707;">Registration & Abstracts</span><br>
  <span style="color:#00693E;">Now Open!</span>
</div>

<div style="margin-top:0.5em; font-size:0.95em; line-height:1.6;">
  📋 <a href="https://www.eventsforce.net/cbs/frontend/reg/thome.csp?pageID=178086&eventID=724&CSPCHD=000001000000RSsr5tSQVu$U89Ob73coFMT_0UIRlG1BnW0nXO" style="color:#00693E; font-weight:bold;">Register here</a><br>
  📝 <a href="https://app.oxfordabstracts.com/auth?redirect=/stages/80645/submitter" style="color:#00693E; font-weight:bold;">Late Abstract Submission Open (poster presentations only)</a><br>
  <span style="color:#D30707; font-weight:bold;">⏰ Abstract & Registration deadline: 14 August 2026</span>
</div>

</div>

:::

# Join us in Cardiff for EMCHD 2026

We are delighted to welcome you to the **20th European Meeting on Complement in Human Disease (EMCHD26)**, taking place from **1st - 5th September 2026** in **Cardiff, Wales**.

This key meeting in the complement field will bring together researchers, clinicians, and industrial delegates from across the globe to explore new research findings in complement biology, including disease mechanisms, biomarker discovery and therapeutic innovations.  
Join us in Cardiff, the capital of Wales, for five days of science, collaboration, and vibrant discussions.

# Our Keynotes

We are excited to welcome world-class keynote speakers:

## Prof Menna Clatworthy

:::: {.grid}
::: {.g-col-12 .g-col-md-6 .order-2 .order-md-1}
![](./images/menna-portrait.svg){.img-fluid width="250"}
:::

::: {.g-col-12 .g-col-md-6 .order-1 .order-md-2}
University of Cambridge, UK  
[Professor Menna Clatworthy FMedSci | Cambridge Immunology Network](https://www.immunology.cam.ac.uk/Networkdirectory/mrc)
:::
::::

## Prof V. Michael Holers

:::: {.grid}
::: {.g-col-12 .g-col-md-6 .order-2 .order-md-1}
![](./images/michael-portrait.svg){.img-fluid width="250"}
:::

::: {.g-col-12 .g-col-md-6 .order-1 .order-md-2}
University of Colorado, USA  
[https://som.cuanschutz.edu/Profiles/Faculty/Profile/4950](https://som.cuanschutz.edu/Profiles/Faculty/Profile/4950)
:::
::::

## Prof Baljit S. Khakh

:::: {.grid}
::: {.g-col-12 .g-col-md-6 .order-2 .order-md-1}
![](./images/bal_portrait.svg){.img-fluid width="250"}
:::

::: {.g-col-12 .g-col-md-6 .order-1 .order-md-2}
Cardiff University, UK  
[https://www.ukdri.ac.uk/team/baljit-s-khakh](https://www.ukdri.ac.uk/team/baljit-s-khakh)
:::
::::

## Prof William Gray

:::: {.grid}
::: {.g-col-12 .g-col-md-6 .order-2 .order-md-1}
![](./images/liam_portrait.svg){.img-fluid width="250"}
:::

::: {.g-col-12 .g-col-md-6 .order-1 .order-md-2}
Cardiff University, UK  
[https://healthandcareresearchwales.org/gray](https://healthandcareresearchwales.org/gray)
:::
::::

# Abstract Submission

Abstracts are submitted via **Oxford Abstracts**, and accepted abstracts, along with Invited Reviews, will be published in a Special Issue of **Clinical & Experimental Immunology (Wiley)**.

Abstract Submission is now open until the 30th of April 2026: [https://app.oxfordabstracts.com/auth?redirect=/stages/80645/submitter](https://app.oxfordabstracts.com/auth?redirect=/stages/80645/submitter)

# Registration

## Main Conference [1st – 5th Sep 2026]

Registration is now open here: [https://www.eventsforce.net](https://www.eventsforce.net/cbs/frontend/reg/thome.csp?pageID=178086&eventID=724&CSPCHD=000001000000RSsr5tSQVu$U89Ob73coFMT_0UIRlG1BnW0nXO)

## Satellite Symposium: Complement and the Brain [1st Sep 2026]

We are delighted to host a satellite symposium on 'Complement and the Brain', running in parallel with the Teaching Day (on Tuesday 1st September 2026). 
This symposium will focus on the emerging roles of complement in neuroinflammation and neurodegenerative diseases, with presentations ranging from fundamental mechanisms to therapeutic developments.

In the late afternoon of the satellite symposium the keynote will be delivered by Professor Baljit Khakh (Centre Director & Research Division Lead, UK Dementia Research Institute).

We are grateful to the UK DRI for supporting this event. 

Registration is open to all (including those not attending the main conference).

## Social Excursions [4th Sep 2026, afternoon]

Enjoy the best of Welsh history and hospitality; options for trips will include:

- Cardiff Castle tour [https://www.cardiffcastle.com/](https://www.cardiffcastle.com/)
- National Museums of Wales Cardiff [https://museum.wales/cardiff/](https://museum.wales/cardiff/)
- St Fagans National Museum of History [https://museum.wales/stfagans/](https://museum.wales/stfagans/)
- City walking tour

All excursions will be available to select during registration on a first come, first served basis. For ECR social events, please see the ECR Activities tab.

## Interactive map of locations

<iframe width="100%" height="576" src="https://maphub.net/embed_h/vif0YthoOKyAwxmA?panel=1&panel_closed=1" frameborder="0"></iframe>

### We look forward to welcoming you

Cardiff is a dynamic capital city, rich in culture and history – the perfect setting to host our international complement community. 
We’re excited to welcome you to EMCHD2026 to share science, ideas, and a little Welsh hospitality.

{{< video https://www.youtube.com/watch?v=UqicqIMGGpI >}}

# Our Sponsors

We thank our sponsors for their generous support in making this meeting possible.

## Gold Sponsor

[![](images/Svar-logo.svg){fig-align="center" width=120%}](https://www.svarlifescience.com/)

## Silver Sponsors

:::: {.columns}

::: {.column width="50%"}
[![](images/HycultBio.jpg){fig-align="center" width=75%}](https://www.hycultbiotech.com/)
:::

::: {.column width="50%"}
[![](images/quidel-ortho-indigo-white-logo.png){fig-align="center" width=75%}](https://www.quidelortho.com/)
:::

::::

:::: {.columns}

::: {.column width="50%"}
[![](images/compvide_logo.png){fig-align="center" width=75%}](https://www.compvide.com)
:::

::: {.column width="50%"}
[![](./images/exserabiolabs_logo.png){fig-align="center" width=75%}](https://www.exserabiolabs.org)
:::

::::

## Bronze Sponsors

[![](images/sobi_logo.jpeg){fig-align="center" width=40%}](https://www.sobi.com/en)

[![](images/CompTech_Logo_2025.jpeg){fig-align="center" width=40%}](https://www.complementtech.com/)

[![](../images/AZ_Alexion_FINAL_LOCKUP_RGB_Positive.png){fig-align="center" width=40%}](https://alexion.com/worldwide/UK/)

## Poster Refreshment

[![](./images/perceive_biotherapeutics_logo.jpg){fig-align="center" width=40%}](https://perceivebio.com)

## Satellite Symposium Sponsor

[![](images/UK_DRI_Logo_Pack_2025/UK_DRI_Logo_Core/PNG/logo.png){fig-align="center" width=40%}](https://www.ukdri.ac.uk/)

**See you in Cardiff!**

The EMCHD2026 Local Organising Committee

